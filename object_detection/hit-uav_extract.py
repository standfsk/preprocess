...
def extract(split, input_path, output_path):
    ...
    image_data = {}
    label_data = {}
    with open(input_path/f"normal_json/annotations/{split}.json", "r") as json_file:
        json_data = json.load(json_file)
        for image_info in json_data['images']:
            image_id = int(image_info['id'])
            image_file_name = image_info['filename']
        ...
        for annotation_info in json_data['annotation']:
        ...
        for image_file_name, lines in tqdm(label_data.items(), desc=f"processing {split}"):
            # image
            shutil.copy(input_path/f"normal_json/{split}/{image_file_name}",
...
if __name__ == "__main__":
    input_path = Path("dataset/HIT-UAV")
    output_path = Path("dataset/HIT-UAV")
